/* tslint:disable */
/* eslint-disable */
/**
 * rds_mysql_v2
 * No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)
 *
 * OpenAPI spec version: common-version
 * 
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */

import { AssociatedInstanceForDescribeAllowListDetailOutput } from './associated-instance-for-describe-allow-list-detail-output';
import { SecurityGroupBindInfoForDescribeAllowListDetailOutput } from './security-group-bind-info-for-describe-allow-list-detail-output';
 /**
 * 
 *
 * @export
 * @interface DescribeAllowListDetailResponse
 */
export interface DescribeAllowListDetailResponse {

    /**
     * @type {string}
     * @memberof DescribeAllowListDetailResponse
     */
    AllowList?: string;

    /**
     * @type {string}
     * @memberof DescribeAllowListDetailResponse
     */
    AllowListCategory?: string;

    /**
     * @type {string}
     * @memberof DescribeAllowListDetailResponse
     */
    AllowListDesc?: string;

    /**
     * @type {string}
     * @memberof DescribeAllowListDetailResponse
     */
    AllowListId?: string;

    /**
     * @type {string}
     * @memberof DescribeAllowListDetailResponse
     */
    AllowListName?: string;

    /**
     * @type {string}
     * @memberof DescribeAllowListDetailResponse
     */
    AllowListType?: string;

    /**
     * @type {Array<AssociatedInstanceForDescribeAllowListDetailOutput>}
     * @memberof DescribeAllowListDetailResponse
     */
    AssociatedInstances?: Array<AssociatedInstanceForDescribeAllowListDetailOutput>;

    /**
     * @type {string}
     * @memberof DescribeAllowListDetailResponse
     */
    ProjectName?: string;

    /**
     * @type {Array<SecurityGroupBindInfoForDescribeAllowListDetailOutput>}
     * @memberof DescribeAllowListDetailResponse
     */
    SecurityGroupBindInfos?: Array<SecurityGroupBindInfoForDescribeAllowListDetailOutput>;

    /**
     * @type {string}
     * @memberof DescribeAllowListDetailResponse
     */
    UserAllowList?: string;
}
