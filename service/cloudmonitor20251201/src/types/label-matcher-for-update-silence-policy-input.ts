/* tslint:disable */
/* eslint-disable */
/**
 * cloudmonitor20251201
 * No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)
 *
 * OpenAPI spec version: common-version
 * 
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */

import { MatcherForUpdateSilencePolicyInput } from './matcher-for-update-silence-policy-input';
 /**
 * 
 *
 * @export
 * @interface LabelMatcherForUpdateSilencePolicyInput
 */
export interface LabelMatcherForUpdateSilencePolicyInput {

    /**
     * @type {Array<MatcherForUpdateSilencePolicyInput>}
     * @memberof LabelMatcherForUpdateSilencePolicyInput
     */
    Matchers?: Array<MatcherForUpdateSilencePolicyInput>;
}
